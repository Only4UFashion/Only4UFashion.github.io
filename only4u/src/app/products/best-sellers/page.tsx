...
async function fetchBestSellers(): Promise<Product[]> {
  try {
    ...
    console.error('Unexpected error fetching best sellers:', err);
    return [];
  }
}
  ...
  useEffect(() => {
    async function loadProducts() {
      ...
      try {
        ...
        setError(validProducts.length === 0 ? 'No best sellers found' : '');
      } catch (err) {
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
              {products.length > 0 ? (
                ...
                <p className="text-center text-gray-600 col-span-full">
                  {error ? 'Failed to load best sellers' : 'No best sellers available.'}
                </p>
              )}
            </div>
          </div>
